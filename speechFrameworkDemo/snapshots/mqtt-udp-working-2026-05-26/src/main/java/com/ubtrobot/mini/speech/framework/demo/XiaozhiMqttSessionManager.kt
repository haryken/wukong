    ...
    private fun performFullWakeReconnect() {
        ...
        wakeReconnectJob = scope.launch {
            try {
                ...
                Log.i(TAG, "[WakeWord] Hey mini: MQTT+UDP + listen OK – chờ TTS chào hoặc timeout mở PCM")
                scope.launch {
                    delay(6000)
                    if (suppressServerPcmUntilFirstGreetingDone && protocol.isAudioChannelOpened()) {
                        Log.w(TAG, "[Greeting] 6s không có TTS chào – mở PCM (tránh kẹt im lặng)")
