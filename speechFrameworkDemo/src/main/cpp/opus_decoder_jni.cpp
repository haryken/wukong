...
Java_info_dourok_voicebot_OpusDecoder_nativeDecodeBytes(JNIEnv *env, jobject thiz,
    ...
    if (input_buffer == nullptr || output_buffer == nullptr || max_output_size < 2) {
        LOGE("Invalid buffer");
        return -1;
    }
    if (input_size < 0) {
        LOGE("Invalid input_size");
        return -1;
    }
    ...
    if (input == nullptr || output == nullptr) {
        if (input) env->ReleaseByteArrayElements(input_buffer, input, JNI_ABORT);
        if (output) env->ReleaseByteArrayElements(output_buffer, output, JNI_ABORT);
        return -1;
    }
